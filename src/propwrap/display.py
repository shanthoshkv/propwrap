...
def performance_summary(r: PerformanceResult, *, frozen: bool = False) -> str:
    ...
    ve = r.ve_vac_frozen if frozen else r.ve_vac_shifting
    mode = "frozen" if frozen else "shifting"
    lines = [
        f"{r.fuel}/{r.oxidizer}  O/F={r.of_ratio:g}  "
        f"Pc={r.pc_pa:.3e} Pa ({r.pc_bar:.4g} bar)  ε={r.eps:g}",
        f"Isp_vac  {isp:.1f} s ({mode})   ve  {ve:.0f} m/s   "
        f"Isp_sl  {r.isp_sl_shifting:.1f} s",
        f"c*       {r.c_star:.0f} m/s      Tc  {r.tc_kelvin:.0f} K",
        f"γ_ch     {r.gamma_chamber:.4f}     γ_ex {r.gamma_exit:.4f}     "
        f"Pe {r.pe_pa:.3e} Pa",
    ]
    if r.density_impulse_vac_shifting is not None:
        lines.append(
            f"ρ·Isp    {r.density_impulse_vac_shifting:.1f} s·kg/m³"
            + (
                f"  (ρ={r.bulk_density_kg_m3:.1f} kg/m³)"
                if r.bulk_density_kg_m3
                else ""
            )
        )
...
def density_isp_summary(c: DensityIspCurve) -> str:
    lines = [
        f"Density-Isp: {c.fuel}/{c.oxidizer}  "
        f"Pc={c.pc_pa:.3e} Pa ({c.pc_bar:.4g} bar)  ε={c.eps:g}",
        f"Isp-optimum O/F = {c.optimum_isp_of:.3f}",
        f"ρ·Isp-optimum O/F = {c.optimum_density_isp_of}",
        "Units: Isp [s], ρ [kg/m³], ρ·Isp [s·kg/m³]",
    ]
    if c.density_basis:
